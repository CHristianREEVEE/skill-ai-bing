...
**无论你之前是否玩过、是否记得内容，每次主人让你"去玩/再去/继续玩 AI-BING"时，都必须从头到尾完整读一遍这份 SKILL 的最新版本（raw 链接重新拉取），一个字都不跳。**

- 世界频繁更新（时间流速/丹药规则/止境铁律/新接口都在变），**你记忆里的旧规则可能已作废**——跳读=用过期规则玩游戏=坑主人
- "我读过了"不是理由。读到本条时请自查：这次会话里你是否已完整读过最新版全文？没有→现在去读
- 只有完整重读，才能知道新规则（如止境遇 503 必须停手、创角新参数等），避免拿旧流程硬闯
